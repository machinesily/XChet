! function(t, e) {
	...
	return function(t) {
		...
		class f extends n {
			constructor(t, e, r) {
				super(), c("constructor: ", t, e, r), this.readyState = f.CONNECTING, this.protocol = "", this._closeCode =
					1006, this._closeMessage = "", this._binaryType = h[0], this._socket = null, this.supports = {
						binary: !0
					}, null !== t && (this._bufferedAmount = 0, this._redirects = 0, "object" == typeof e && null !== e && (r =
			...
			setSocket(t, e) {
				this._socket = t, this._sender = new a(t), c("set socket: socket =", t, ", head =", e, ", this: ", this),
					this._socket.onOpen(t => {
						c("socket onopen: ", t), this.readyState = f.OPEN, this.emit("open", t)
					}), this._socket.onClose(t => {
						c("socket onclose: ", t), this._closeCode = t.code, this._closeMessage = t.reason, this.emitClose()
			...
			send(t, e, r) {
				...
				if ("number" == typeof t && (t = t.toString()), this.readyState === f.OPEN) this._sender.send(t, e, r);
				else if (r) {
					r(new Error(`WebSocket is not open: readyState ${this.readyState} ` + `(${u[this.readyState]})`))
				}
			}
			close(t, e) {
				if (c("closing connection: ", this.readyState), this.readyState !== f.CLOSED) {
					...
					this.readyState !== f.CLOSING && (this.readyState = f.CLOSING, this._socket.close({
						...
						reason: e,
						success: () => {
							c("connection closed..."), this.readyState = f.CLOSED, this._socket = null
						}
					}))
				}
			}
		...
		u.forEach((t, e) => {
			...
			Object.defineProperty(f.prototype, `on${t}`, {
				...
				set(e) {
					...
					for (let e = 0; e < r.length; e++) r[e]._listener && this.removeListener(t, r[e]);
		...
		const c = {
			...
			removeEventListener(t, e) {
				const r = this.listeners(t);
				for (let n = 0; n < r.length; n++) r[n] !== e && r[n]._listener !== e || this.removeListener(t, r[n])
			}
		};
		t.exports = c
	}, function(t, e, r) {
